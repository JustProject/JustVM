...
namespace zipper {
    ...
    private:
        ...
        ZipEntry currentEntryInfo() {
            ...
            int err = unzGetCurrentFileInfo64(m_zf, &file_info, filename_inzip, sizeof(filename_inzip), NULL, 0, NULL,
                                              0);
            if (UNZ_OK != err)
            ...
            return ZipEntry(std::string(filename_inzip), file_info.compressed_size, file_info.uncompressed_size,
                            ...
                            file_info.tmu_date.tm_hour, file_info.tmu_date.tm_min, file_info.tmu_date.tm_sec,
                            file_info.dosDate);
        }
    ...
    public:
        ...
        bool extractAll(const std::string &destination, const std::map<std::string, std::string> &alternativeNames) {
            iterEntries(
                    [this, &destination, &alternativeNames](ZipEntry &entryinfo) {
                        ...
                        std::function<int(ZipEntry &)> func = std::bind(&zipper::Unzipper::Impl::extractToFile, this,
                                                                        alternativeName, std::placeholders::_1);
        ...
        bool extractEntry(const std::string &name, const std::string &destination) {
            ...
            std::function<int(ZipEntry &)> func = std::bind(&zipper::Unzipper::Impl::extractToFile, this, outputfile,
                                                            std::placeholders::_1);
        ...
        bool extractEntryToStream(const std::string &name, std::ostream &stream) {
            std::function<int(ZipEntry &)> func = std::bind(&zipper::Unzipper::Impl::extractToStream, this,
        ...
        bool extractEntryToMemory(const std::string &name, std::vector<unsigned char> &vec) {
            std::function<int(ZipEntry &)> func = std::bind(&zipper::Unzipper::Impl::extractToMemory, this,
    ...
    Unzipper::Unzipper(std::istream &zippedBuffer)
            ...
            , m_usingMemoryVector(false), m_usingStream(true), m_impl(new Impl(*this)) {
    ...
    Unzipper::Unzipper(std::vector<unsigned char> &zippedBuffer)
            ...
            , m_vecbuffer(zippedBuffer), m_usingMemoryVector(true), m_usingStream(false), m_impl(new Impl(*this)) {
    ...
    Unzipper::Unzipper(const std::string &zipname)
            : m_ibuffer(*(new std::stringstream())) //not used but using local variable throws exception
            , m_vecbuffer(*(new std::vector<unsigned char>())) //not used but using local variable throws exception
            , m_zipname(zipname), m_usingMemoryVector(false), m_usingStream(false), m_impl(new Impl(*this)) {
    ...
    Unzipper::Unzipper(const std::string &zipname, const std::string &password)
            : m_ibuffer(*(new std::stringstream())) //not used but using local variable throws exception
            , m_vecbuffer(*(new std::vector<unsigned char>())) //not used but using local variable throws exception
            , m_zipname(zipname), m_password(password), m_usingMemoryVector(false), m_usingStream(false),

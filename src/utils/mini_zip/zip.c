...
local int add_data_in_datablock OF((linkedlist_data * ll,
...
local int zipGetDiskSizeAvailable OF((zipFile
                                             file, ZPOS64_T * size_available));
...
local int zipGoToSpecificDisk OF((zipFile
                                         file,
...
local int zipGoToFirstDisk OF((zipFile
                                      file));
...
local int zipGoToNextDisk OF((zipFile
                                     file));
...
#endif
    zi->ci.central_header = (char *) ALLOC(
            (uInt) zi->ci.size_centralheader + zi->ci.size_centralextrafree + size_comment);
...
extern int ZEXPORT zipOpenNewFileInZip2(zipFile file, const char *filename, const zip_fileinfo *zipfi,
                                        ...
                                        int raw) {
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
                                   extrafield_global, size_extrafield_global, comment, method, level, raw, -MAX_WBITS,
                                   DEF_MEM_LEVEL,
...
extern int ZEXPORT zipOpenNewFileInZip64(zipFile file, const char *filename, const zip_fileinfo *zipfi,
                                         ...
                                         int zip64) {
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
                                   extrafield_global, size_extrafield_global, comment, method, level, 0, -MAX_WBITS,
                                   DEF_MEM_LEVEL,
                                   Z_DEFAULT_STRATEGY, NULL, 0, VERSIONMADEBY, 0, zip64);
}
...
extern int ZEXPORT zipCloseFileInZipRaw64(zipFile file, ZPOS64_T uncompressed_size, uLong crc32) {
    ...
    if (compressed_size >= 0xffffffff || uncompressed_size >= 0xffffffff || zi->ci.pos_local_header >= 0xffffffff) {
        ...
        uLong versionMadeBy = (uLong) zip64local_getValue_frommemory(zi->ci.central_header + 4,
                                                                     2); /* version made by */
        if ((versionMadeBy & 0xff) < 45)
            zip64local_putValue_inmemory(zi->ci.central_header + 4, (uLong) ((versionMadeBy & 0xff00) | (uLong) 45),
                                         2); /* version made by */

        ...
        voidpf stream;
{
    ...
    while (bytesLeftToWrite > 0) {
        if (bufio->filefunc64.zwrite_file != NULL)
            bytesWritten = bufio->filefunc64.zwrite_file(bufio->filefunc64.opaque, streamio->stream,
                                                         streamio->writeBuffer + (bytesToWrite - bytesLeftToWrite),
                                                         bytesLeftToWrite);
        else
            bytesWritten = bufio->filefunc.zwrite_file(bufio->filefunc.opaque, streamio->stream,
                                                       streamio->writeBuffer + (bytesToWrite - bytesLeftToWrite),
                                                       bytesLeftToWrite);
    ...
    while (bytesLeftToRead > 0) {
        if ((streamio->readBufferLength == 0) || (streamio->readBufferPos == streamio->readBufferLength)) {
            ...
            if (bufio->filefunc64.zread_file != NULL)
                bytesRead = bufio->filefunc64.zread_file(bufio->filefunc64.opaque, streamio->stream,
                                                         streamio->readBuffer + streamio->readBufferPos, bytesToRead);
            else
                bytesRead = bufio->filefunc.zread_file(bufio->filefunc.opaque, streamio->stream,
                                                       streamio->readBuffer + streamio->readBufferPos, bytesToRead);
            ...
            print_buf(opaque, stream, "filled [read %d/%d buf %d:%d pos %lld]\n", bytesRead, bytesToRead,
                      streamio->readBufferPos, streamio->readBufferLength, streamio->position);
        ...
        if ((streamio->readBufferLength - streamio->readBufferPos) > 0) {
            ...
            print_buf(opaque, stream, "emptied [copied %d remaining %d buf %d:%d pos %lld]\n", bytesToCopy,
                      bytesLeftToRead, streamio->readBufferPos, streamio->readBufferLength, streamio->position);
        }
    }
        ...
        uLong size;
{
    ...
    if (streamio->readBufferLength > 0) {
        ...
        if (bufio->filefunc64.zseek64_file != NULL)
            retVal = bufio->filefunc64.zseek64_file(bufio->filefunc64.opaque, streamio->stream, streamio->position,
                                                    ZLIB_FILEFUNC_SEEK_SET);
        else
            retVal = bufio->filefunc.zseek_file(bufio->filefunc.opaque, streamio->stream, (uLong) streamio->position,
                                                ZLIB_FILEFUNC_SEEK_SET);
    ...
    while (bytesLeftToWrite > 0) {
        bytesToCopy = min(bytesLeftToWrite,
                          (uLong) (IOBUF_BUFFERSIZE - min(streamio->writeBufferLength, streamio->writeBufferPos)));
        ...
        memcpy(streamio->writeBuffer + streamio->writeBufferPos, (char *) buf + (bytesToWrite - bytesLeftToWrite),
               bytesToCopy);

        print_buf(opaque, stream, "write copy [remaining %d write %d:%d len %d]\n", bytesToCopy, bytesToWrite,
                  bytesLeftToWrite, streamio->writeBufferLength);
        ...
        ZPOS64_T position;
{
    ...
    print_buf(opaque, stream, "tell [pos %llu readpos %d writepos %d err %d]\n", streamio->position,
              streamio->readBufferPos, streamio->writeBufferPos, errno);
    if (streamio->readBufferLength > 0)
        ...
        int origin;
{
    ...
    switch (origin) {
        case ZLIB_FILEFUNC_SEEK_SET:
            ...
            if ((streamio->readBufferLength > 0) && (offset < streamio->position) &&
                (offset >= streamio->position - streamio->readBufferLength)) {
        ...
        voidpf stream;
{
    ...
    if (streamio->readBufferHits + streamio->readBufferMisses > 0)
            print_buf(opaque, stream, "read efficency %.02f%%\n",
                      (streamio->readBufferHits / ((float) streamio->readBufferHits + streamio->readBufferMisses)) *
                      100);
    if (streamio->writeBufferHits + streamio->writeBufferMisses > 0)
            print_buf(opaque, stream, "write efficency %.02f%%\n",
                      (streamio->writeBufferHits / ((float) streamio->writeBufferHits + streamio->writeBufferMisses)) *
                      100);
